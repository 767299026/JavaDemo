package com.lsl.auth.support.core;

import cn.hutool.core.util.StrUtil;
import cn.hutool.extra.spring.SpringUtil;
import com.lsl.security.service.UserDetailService;
...
public class DaoAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider {
	...
	protected final UserDetails retrieveUser(String username, UsernamePasswordAuthenticationToken authentication) {
		...
		String clientId = WebUtil.getRequest().get().getParameter(OAuth2ParameterNames.CLIENT_ID);

		if (StrUtil.isBlank(clientId)) {
			clientId = basicConvert.convert(request).getName();
		}

		Map<String, UserDetailService> userDetailsServiceMap = SpringUtil
				.getBeansOfType(UserDetailService.class);

		String finalClientId = clientId;
		Optional<UserDetailService> optional = userDetailsServiceMap.values()
				.stream()
